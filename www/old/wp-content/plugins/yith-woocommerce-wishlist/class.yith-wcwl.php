...
if( !class_exists( 'YITH_WCWL' ) ) {    
    ...
    class YITH_WCWL {
        ...
        public function add() {
            ...
            if ( isset( $this->details['add_to_wishlist'] ) && is_numeric( $this->details['add_to_wishlist'] ) ) {
                ...
                if( $return ) {
                    ...
                    $this->errors[] = __( 'Opa! Deu algo errado enquanto o produto era adicionado na lista Quero! :(', 'yit' );
                    return "error";
                }
            } 
        }
        ...
        public function remove( $id ) {
            ...
            if( is_user_logged_in() ) {
                ...
                if( $wpdb->query( $sql ) ) {
                    ...
                    $this->errors[] = __( 'Opa! Deu algo errado enquanto o produto era removido da lista Quero! :(', 'yit' );
                    return false;
                }
            } elseif( yith_usecookies() ) {
        ...
        public function add_to_wishlist_ajax() {    
            ...
            if( $return == 'true'  )
                { echo $return . '##' . __( 'Produto Adicionado!', 'yit' ); }
            elseif( $return == 'exists' )
                { echo $return . '##' . __( 'O produto já está na lista Quero!', 'yit' ); }
        ...
        public function remove_from_wishlist_ajax() {   
            ...
            if( $this->remove( $_GET['wishlist_item_id'] ) )
                { echo apply_filters( 'yith_wcwl_product_removed_text', __( 'Produto removido.', 'yit' ) ); }
            else {
                echo '#' . $count . '#';
                _e( 'Opa! Deu algo errado, não conseguimos remover o produto. :(', 'yit' );
            }
            
            if( !$count )
                { _e( 'Nenhum produto foi adicionado à lista Quero!', 'yit' ); }
