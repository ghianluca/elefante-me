    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        <thead>
            <tr>
                ...
                <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( 'preço', 'yit' ) ?></span></th><?php endif ?>
                <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th><span class="nobr"><?php _e( 'Estoque', 'yit' ) ?></span></th><?php endif ?>
            ...
            if( count( $wishlist ) > 0 ) :
                foreach( $wishlist as $values ) :   
                    ...
                    <tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
                        <td class="product-remove"><div><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remover este produto', 'yit' ) ?>"><div class="icon-close"></div></a></td>
                        ...
                        <td class="product-stock-status">
                            ...
                            if( $stock_status == 'out-of-stock' ) {
                                ...
                                $stock_status = "In";
                                echo '<span class="wishlist-in-stock">' . __( 'No estoque!', 'yit' ) . '</span>';
                            }
                            ?>
                        </td>
                        <?php endif ?>
                ...
                <tr>
                    <td colspan="6" class="wishlist-empty"><?php _e( 'Nenhum produto foi adicionado à lista Quero!', 'yit' ) ?></td>
                </tr>       
            <?php
            endif;
